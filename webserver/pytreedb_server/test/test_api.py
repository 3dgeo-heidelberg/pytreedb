...
def test_get_exportcsv(myserver, session):
    ...
    response = session.get(f"{host}:{port}/download/exportcsv/{base64.b64encode(json.dumps(query['data']).encode()).decode()}")
    assert response.status_code == 200
...
def test_get_exportcollection(myserver, session):
    ...
    response = session.get(f"{host}:{port}/download/exportcollection/{base64.b64encode(json.dumps(query['data']).encode()).decode()}")
    assert response.status_code == 200
...
def test_get_lazlinks(myserver, session):
    ...
    response = session.get(f"{host}:{port}/download/lazlinks/{base64.b64encode(json.dumps(query['data']).encode()).decode()}")
    assert response.status_code == 200
    # TODO: assert much more, e.g. results
