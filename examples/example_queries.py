...
print(f" - Tree properties that are shared among all trees "
...
print(f" - Get list of a specific field for all trees fulfilling slicing:"
...
print(" - Query by key and value without regex (key=\"properties.species\", value=\"Quercus petraea\"): ",
      len(mydb.query_by_key_value(key="properties.species", value="Quercus petraea", regex=False)))
print(" - Query by key and value with regex (key=\"properties.species\", value=\"Quercus *\"): ",
      len(mydb.query_by_key_value(key="properties.species", value="Quercus *", regex=True)))

print("\n# Query by date")
print(" - Query with start and end date"
      "(key=\"properties.measurements.date\", start=\"2019-08-28\", end=\"2022-01-01\"):",
      len(mydb.query_by_date(key="properties.measurements.date", start="2019-08-28", end='2022-01-01')))
print(" - Query with only start date (key=\"properties.data.date\", start=\"2019-08-28\"):",
      len(mydb.query_by_date(key="properties.data.date", start="2019-08-28")))

print("\n# Query by search geometry [watch out LatLon requirement]")
print(" - Distance search around point (2000m): ",
      len(mydb.query_by_geometry(json.loads('{"type": "Point", "coordinates": [ 8.682112, 49.014387] }'), distance=2000.0)))
print(" - Distance search around point (1500m): ",
      len(mydb.query_by_geometry('{"type": "Point", "coordinates": [8.682112, 49.014387]}', distance=1500.0)))
print(" - Query by search geometry, here point-in-polygon: ",
      len(mydb.query_by_geometry('{ "type": "Polygon", '
                                 '"coordinates": [['
...
trees_in_poly = mydb.query_by_geometry('{ "type": "Polygon", '
                                       '"coordinates": [['
                                       '[8.700980940620983, 49.012725603975355],'
                                       '[8.700972890122355, 49.011695140150906],'
                                       '[8.70235623413669, 49.01167501390433],'
...
print(mydb.get_ids(mydb.query({"$or": [{"properties.species": "Abies alba"},
...
print(mydb.get_ids(mydb.query({"$and": [{"properties.species": "Abies alba"},
                                        {"$or": [{"properties.data.mode": "TLS"}, {"properties.data.mode": "ALS"}]}]})))
...
print(" - Write/Export selected trees as GeoJSON files to given output directory:"
